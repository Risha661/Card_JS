...
export function validateNumber(value) {
  if (!value) return "Данные невалидны";
  const cleaned = value.replace(/\s+/g, "");
  return /^\d{16}$/.test(cleaned) ? value : "Данные невалидны";
}

export function validateExpiry(value) {
  if (!value) return "Данные невалидны";
  return /^(0[1-9]|1[0-2])\/\d{2}$/.test(value) ? value : "Данные невалидны";
}

export function validateCVV(value) {
  if (!value) return "Данные невалидны";
  return /^\d{3}$/.test(value) ? value : "Данные невалидны";
}
